...
public class DragGridView extends GridView {
	 ...
	 * 是否可以拖拽，默认不可以
	 */
	public static boolean isDragabel = false;
	...
	private final Runnable mLongClickRunnable = new Runnable() {
		...
		@Override
		public void run() {
			if (isLongClick && !isDragabel && mStartDragItemView != null) {
				// && MainViewPager.currentItem ==
				// MainActivity.POSITION_CATEGORY) {
				isDragabel = true; // 设置可以拖拽
				isLongClick = false;
	...
	private boolean isMove(int x, int y) {
		int moveX = Math.abs(mDownX - x);
		int moveY = Math.abs(mDownY - y);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if (isDragabel && mDragImageView != null) {
			switch (ev.getAction()) {
			case MotionEvent.ACTION_MOVE:
				moveX = (int) ev.getX();
				moveY = (int) ev.getY();
			...
			case MotionEvent.ACTION_UP:
				onStopDrag();
				isDragabel = false;
				// ImageItemAdapter.mHidePosition = -1;
				break;
			}
			return true;
		}
	...
	private void onSwapItem(int moveX, int moveY) {
		...
		if (tempPosition != mDragPosition
			...
			View temp1 = getChildAt(tempPosition - getFirstVisiblePosition());
			View temp2 = getChildAt(mDragPosition - getFirstVisiblePosition());
			if (temp1 != null && temp2 != null) {
				temp1.setVisibility(View.INVISIBLE);// 拖动到了新的item,新的item隐藏掉
				temp2.setVisibility(View.VISIBLE);// 之前的item显示出来
			}
