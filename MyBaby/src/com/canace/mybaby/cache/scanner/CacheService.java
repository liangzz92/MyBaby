/**
 * CacheService.java
 * CacheService工作机制包括三个层面的操作：
 * 初始化：遍历系统MediaStore提供的媒体文件及thumbnails信息，将相关信息整理并存储至SDCard
 * 文件变化监听：监听含有媒体文件的相关文件夹，添加或删除相应记录
 * 本地检测：遍历完毕后开启线程，进行人脸检测等后续准备操作
 * 
 * 2014-12-13 下午2:08:36
 * 
 * @author 悦
 */
...
import com.canace.mybaby.cache.model.DataSource;
import com.canace.mybaby.cache.model.DiskCache;
import com.canace.mybaby.cache.model.ImageList;
...
import com.canace.mybaby.cache.utils.CacheUtils;
import com.canace.mybaby.cache.utils.Shared;
import com.canace.mybaby.cache.utils.SortCursor;
import com.canace.mybaby.cache.utils.UriTexture;
...
public final class CacheService extends IntentService {
	public static final String ACTION_CACHE = "com.canace.mybaby.cache.scanner.action.CACHE";
	public static final DiskCache sAlbumCache = new DiskCache(
			"local-album-cache");
	public static final DiskCache sMetaAlbumCache = new DiskCache(
			"local-meta-cache");
	public static final DiskCache sSkipThumbnailIds = new DiskCache(
			"local-skip-cache");
	...
	private static final int ALBUM_CACHE_NONEIMAGE_INDEX = -6;

	private static final DateFormat mDateFormat = new SimpleDateFormat(
			"yyyy:MM:dd HH:mm:ss");
	private static final DateFormat mAltDateFormat = new SimpleDateFormat(
	...
	/**
	 * 生成并返回diskCache文件存储路径
	 * 
	 * @param subFolderName
	 * @return
	 */
	public static final String getCachePath(final String subFolderName) {
		return FileHelper.getCacheRootDirectory() + subFolderName + '/';
	}

	/**
	 * 检测系统区域信息以指定语言编码，开启媒体信息缓存服务 将由handleIntent()函数执行后续操作
	 * 
	 * @param context
	 * @param checkthumbnails
	 */
	public static final void startCache(final Context context,
			final boolean checkthumbnails) {
		...
		final Intent intent = new Intent(ACTION_CACHE, null, context,
				CacheService.class);
	...
	/**
	 * check whether the CacheService has run over, including the cache
	 * scanning, local detecting and also the online detecting
	 * 
	 * @param onlyMediaSets
	 * @return whether the Cache is ready
	 */
	public static final boolean isCacheReady(final boolean onlyMediaSets) {
		if (onlyMediaSets) {
			boolean ALBUM_CACHE_METADATA_INDEX_flag = (sAlbumCache.get(
					ALBUM_CACHE_METADATA_INDEX, 0) != null || sAlbumCache.get(
					ALBUM_CACHE_NONEIMAGE_INDEX, 0) != null);
			boolean ALBUM_CACHE_DIRTY_INDEX_flag = (sAlbumCache.get(
					ALBUM_CACHE_DIRTY_INDEX, 0) == null);
			return ALBUM_CACHE_METADATA_INDEX_flag
					&& ALBUM_CACHE_DIRTY_INDEX_flag;
		} else {
			...
			boolean ALBUM_CACHE_DIRTY_INDEX_flag = (sAlbumCache.get(
					ALBUM_CACHE_DIRTY_INDEX, 0) == null);
			boolean ALBUM_CACHE_INCOMPLETE_INDEX_flag = (sAlbumCache.get(
					ALBUM_CACHE_INCOMPLETE_INDEX, 0) == null);
			return (ALBUM_CACHE_METADATA_INDEX_flag
					&& ALBUM_CACHE_DIRTY_INDEX_flag
					&& ALBUM_CACHE_INCOMPLETE_INDEX_flag
					&& !OnlineDetectService.isDetecting() && !LocalDetectService
						.isDetecting());
		}
	}
	...
	public static final boolean isCacheReady(final long setId) {
		final boolean isReady = ((sAlbumCache
				.get(ALBUM_CACHE_METADATA_INDEX, 0) != null || sAlbumCache.get(
				ALBUM_CACHE_NONEIMAGE_INDEX, 0) != null)
				&& sAlbumCache.get(ALBUM_CACHE_DIRTY_INDEX, 0) == null && sAlbumCache
				.get(ALBUM_CACHE_INCOMPLETE_INDEX, 0) == null);
		if (!isReady) {
			return isReady;
		}
		// Also, we need to check if this setId is dirty.
		final byte[] existingData = sAlbumCache.get(
				ALBUM_CACHE_DIRTY_BUCKET_INDEX, 0);
	 ...
	 * @param observer
	 */
	public static final void senseDirty(final Context context,
			...
			restartThread(CACHE_THREAD, "CacheRefresh", new Runnable() {
				...
				public void run() {
					...
					if (ids != null && ids.length > 0) {
						sList = null;
						if (CommonsUtil.DEBUG) {
							Log.i(TAG, "Done computing dirty sets for num "
									+ ids.length);
						}
					}
				}
	...
	/**
	 * 标注指定setId缓存文件需更新, 重新启动Cache服务修改缓存信息
	 * 
	 * @param context
	 * @param id
	 */
	public static final void markDirty(final Context context, final long id) {
		...
		if (CACHE_THREAD.get() == null) {
			QUEUE_DIRTY_SET = false;
			restartThread(CACHE_THREAD, "CacheRefreshDirtySets",
					new Runnable() {
						@Override
						public void run() {
	...
	public static final boolean setHasItems(final ContentResolver cr,
			final long setId) {
		final Uri uriImages = Images.Media.EXTERNAL_CONTENT_URI;
		final StringBuffer whereString = new StringBuffer(
				Images.ImageColumns.BUCKET_ID + "=" + setId);
		try {
			final Cursor cursorImages = cr.query(uriImages,
					BUCKET_PROJECTION_IMAGES, whereString.toString(), null,
					null);
	...
	public static final void loadMediaSets(final MediaFeed feed,
			final DataSource source, final boolean includeImages,
		...
		if (albumData != null && albumData.length > 0) {
			...
			try {
				...
				for (int i = 0; i < numAlbums; ++i) {
					...
					final String name = CacheUtils.readUTF(dis);
	...
	public static final void loadMediaSet(final MediaFeed feed,
			final DataSource source, final long bucketId) {
		int timeElapsed = 0;
		...
		if (albumData != null && albumData.length > 0) {
			...
			try {
				...
				for (int i = 0; i < numAlbums; ++i) {
					...
					mediaSet.mName = CacheUtils.readUTF(dis);
					if (setId == bucketId) {
						mediaSet.mPicasaAlbumId = Shared.INVALID;
						mediaSet.generateTitle(true);
						return;
	...
	public static final void loadMediaItemsIntoMediaFeed(final MediaFeed feed,
			final MediaSet set, final int rangeStart, final int rangeEnd,
		...
		if (albumData != null
				&& set.mNumItemsLoaded < set.getNumExpectedItems()) {
			final DataInputStream dis = new DataInputStream(
					new BufferedInputStream(
							new ByteArrayInputStream(albumData), 256));
			try {
				...
				if (CommonsUtil.DEBUG) {
					Log.i(TAG, "Loading " + numItems + " into set " + set.mName
							+ ":" + set);
				}
				...
				for (int i = 0; i < numItems; ++i) {
					...
					item.mCaption = CacheUtils.readUTF(dis);
					item.mMimeType = CacheUtils.readUTF(dis);
					...
					item.mFilePath = CacheUtils.readUTF(dis);
	...
	public static final void populateVideoItemFromCursor(Context context,
			final MediaItem item, final ContentResolver cr,
			final Cursor cursor, final String baseUri) {
		item.setMediaType(MediaItem.MEDIA_TYPE_VIDEO);
		populateMediaItemFromCursor(context, item, cr, cursor, baseUri);
	}

	public static final void populateMediaItemFromCursor(Context context,
			final MediaItem item, final ContentResolver cr,
			final Cursor cursor, final String baseUri) {
		...
		item.mDateTakenInMs = cursor
				.getLong(CacheService.MEDIA_DATE_TAKEN_INDEX);
		item.mDateAddedInSec = cursor
				.getLong(CacheService.MEDIA_DATE_ADDED_INDEX);
		item.mDateModifiedInSec = cursor
				.getLong(CacheService.MEDIA_DATE_MODIFIED_INDEX);
		...
		final int orientationDurationValue = cursor
				.getInt(CacheService.MEDIA_ORIENTATION_OR_DURATION_INDEX);
		if (itemMediaType == MediaItem.MEDIA_TYPE_IMAGE) {
		...
		if (item.getMediaType() == MediaItem.MEDIA_TYPE_IMAGE
				&& hasNotImageDetected(item)) {
			// getFaceInfo(context, item);
			addLocalDetectImage(context, item);
			setImageDetected(item);
		}
	}
	 ...
	 * @param item
	 */
	private static void setImageDetected(MediaItem item) {
		// TODO Auto-generated method stub
		PreferenceUtils.saveBooleanValue(getItemHashCode(item), false);
	}
	 ...
	 * @param item
	 */
	private static void addLocalDetectImage(Context context, MediaItem item) {
		CommonsUtil.initDBSettings(context);
		DetectImage localDetectImage = new DetectImage();
		localDetectImage.setImagePath(item.mFilePath);
		localDetectImage.setIdHashcode(getItemHashCode(item));
		localDetectImage.setHasLocalDetect(0);
		localDetectImage.setNeedOnlineDetect(0);
		localDetectImage.setItemTimeScore(item.getTimeScore());
		if (!existInDB(localDetectImage)) {
			DBFacade.save(localDetectImage);
			LocalDetectService.addTotalCount();
			if (CommonsUtil.DEBUG) {
				Log.i(TAG,
						"image to local detect:"
								+ localDetectImage.getImagePath());
			}
		} else {
			if (CommonsUtil.DEBUG) {
				Log.i(TAG,
						"same image to local detect:"
								+ localDetectImage.getImagePath());
			}
		}
	}
	 ...
	 * @return
	 */
	private static boolean existInDB(DetectImage localDetectImage) {
		// TODO Auto-generated method stub
		DetectImage[] localDetectImages = (DetectImage[]) DBFacade
				.findByFieldName(DetectImage.class, "id_hashcode",
						localDetectImage.getIdHashcode());
		if (localDetectImages != null && localDetectImages.length > 0) {
			return true;
		}
		return false;
	}

	private static String getItemHashCode(MediaItem item) {
		return String.valueOf((item.mDateAddedInSec + ""
				+ item.mDateModifiedInSec + item.mFilePath).hashCode());
	}
	 ...
	 * @return
	 */
	private static boolean hasNotImageDetected(MediaItem item) {
		// TODO Auto-generated method stub
		return PreferenceUtils.getBooleanValue(getItemHashCode(item));
	}
	...
	public static final long fetchDateTaken(final MediaItem item) {
		if (!item.isDateTakenValid()
				&& !item.mTriedRetrievingExifDateTaken
				&& (item.mFilePath.endsWith(".jpg") || item.mFilePath
						.endsWith(".jpeg"))) {
			try {
				final ExifInterface exif = new ExifInterface(item.mFilePath);
				final String dateTakenStr = exif
				...
				if (dateTakenStr != null) {
					try {
						...
						pe.printStackTrace();
						try {
							final Date dateTaken = mAltDateFormat
									.parse(dateTakenStr);
							return dateTaken.getTime();
							...
							if (CommonsUtil.DEBUG) {
								pe2.printStackTrace();
								Log.i(TAG,
										"Unable to parse date out of string - "
												+ dateTakenStr);
							}
						}
					}
				...
				if (CommonsUtil.DEBUG) {
					e.printStackTrace();
					Log.i(TAG,
							"Error reading Exif information, probably not a jpeg.");
				}
			}
	...
	public static final byte[] queryThumbnail(final Context context,
			final long thumbId, final long origId, final boolean isVideo,
			final long timestamp) {
		final DiskCache thumbnailCache = (isVideo) ? LocalDataSource.sThumbnailCacheVideo
				: LocalDataSource.sThumbnailCache;
		return queryThumbnail(context, thumbId, origId, isVideo,
				thumbnailCache, timestamp);
	}
	...
	public static final ImageList getImageList(final Context context) {
		...
		final Cursor cursorImages = cr.query(uriImages, THUMBNAIL_PROJECTION,
				null, null, null);
		try {
			...
			if (cursorImages != null && cursorImages.moveToFirst()) {
				...
				do {
					...
					timestamp[ctr] = cursorImages
							.getLong(THUMBNAIL_DATE_MODIFIED_INDEX);
					thumbnailIds[ctr] = CacheUtils.Crc64Long(cursorImages
							.getString(THUMBNAIL_DATA_INDEX));
					orientation[ctr] = cursorImages
							.getInt(THUMBNAIL_ORIENTATION_INDEX);
					++ctr;
	...
	private static final byte[] queryThumbnail(final Context context,
			final long thumbId, final long origId, final boolean isVideo,
			final DiskCache thumbnailCache, final long timestamp) {
		if (!((MyBabyActivity) context).isPaused()) {
		...
		if (bitmap == null) {
			...
			bitmap = buildThumbnailForId(context, thumbnailCache, thumbId,
					origId, isVideo, DEFAULT_THUMBNAIL_WIDTH,
					DEFAULT_THUMBNAIL_HEIGHT, timestamp);
			if (CommonsUtil.DEBUG) {
				Log.i(TAG, "Built thumbnail and screennail for " + origId
						+ " in " + (SystemClock.uptimeMillis() - time));
			}
		}
		return bitmap;
	}
	...
	private static final void buildThumbnails(final Context context) {
		...
		for (int i = 0; i < size; ++i) {
			...
			if (!isInThumbnailerSkipList(thumbnailId)) {
				if (!thumbnailCache.isDataAvailable(thumbnailId,
						timeModifiedInSec * 1000)) {
					byte[] retVal = buildThumbnailForId(context,
							thumbnailCache, thumbnailId, id, false,
	...
	private static final byte[] buildThumbnailForId(final Context context,
			final DiskCache thumbnailCache, final long thumbId,
			final long origId, final boolean isVideo, final int thumbnailWidth,
		...
		try {
			...
			if (!isVideo) {
				...
				try {
					bitmap = UriTexture.createFromUri(context, uriString, 1024,
							1024, thumbId, null);
				} catch (IOException e) {
					e.printStackTrace();
					return null;
			...
			final byte[] retVal = writeBitmapToCache(thumbnailCache, thumbId,
					origId, bitmap, thumbnailWidth, thumbnailHeight, timestamp);
			return retVal;
		} catch (InterruptedException e) {
			e.printStackTrace();
			return null;
	...
	public static final byte[] writeBitmapToCache(
			final DiskCache thumbnailCache, final long thumbId,
			final long origId, final Bitmap bitmap, final int thumbnailWidth,
			final int thumbnailHeight, final long timestamp) {
	...
	private static final void putLocaleForAlbumCache(final Locale locale) {
		...
		try {
			CacheUtils.writeUTF(dos, locale.getCountry());
			CacheUtils.writeUTF(dos, locale.getLanguage());
			CacheUtils.writeUTF(dos, locale.getVariant());
			dos.flush();
			bos.flush();
	...
	private static final Locale getLocaleForAlbumCache() {
		...
		if (data != null && data.length > 0) {
			...
			try {
				String country = CacheUtils.readUTF(dis);
				if (country == null)
					country = "";
				String language = CacheUtils.readUTF(dis);
				if (language == null)
					language = "";
				String variant = CacheUtils.readUTF(dis);
				if (variant == null)
					variant = "";
	...
	private static final void restartThread(
			final AtomicReference<Thread> threadRef, final String name,
		...
		try {
			...
			try {
				if (sortCursor != null && sortCursor.moveToFirst()) {
					...
					acceleratedSets = new LongSparseArray<MediaSet>(
							sortCursor.getCount());
					MediaSet cameraSet = new MediaSet();
					cameraSet.mId = LocalDataSource.CAMERA_BUCKET_ID;
					cameraSet.mName = context.getResources().getString(
							R.string.camera);
					sets.add(cameraSet);
					...
					do {
						...
						if (mediaSet == null) {
							mediaSet = new MediaSet();
							mediaSet.mId = setId;
							mediaSet.mName = sortCursor
									.getString(BUCKET_NAME_INDEX);
							sets.add(mediaSet);
							acceleratedSets.put(setId, mediaSet);
						}
						mediaSet.mHasImages |= (sortCursor
								.getCurrentCursorIndex() == 0);
						mediaSet.mHasVideos |= (sortCursor
								.getCurrentCursorIndex() == 1);
	...
	private final static void refreshDirtySets(final Context context) {
		...
		if (existingData != null && existingData.length > 0) {
			...
			if (numIds > 0) {
				...
				final LongSparseArray<MediaSet> acceleratedSets = new LongSparseArray<MediaSet>(
						numIds);
				for (int i = 0; i < numIds; ++i) {
	...
	public static final long[] computeDirtySets(final Context context) {
		...
		try {
			final Cursor cursorImages = cr.query(uriImages, SENSE_PROJECTION,
					where, null, null);
			final Cursor cursorVideos = cr.query(uriVideos, SENSE_PROJECTION,
					where, null, null);
			Cursor[] cursors = new Cursor[2];
			cursors[0] = cursorImages;
			cursors[1] = cursorVideos;
				...
				if (cursor.moveToFirst()) {
					...
					do {
						...
						if (allDirty) {
							...
							boolean contains = sAlbumCache.isDataAvailable(
									setId, 0);
							if (!contains) {
							...
							if (!allDirty) {
								...
								if (maxAdded > oldMaxAdded || oldCount != count) {
									...
									dataLong[0] = maxAdded;
									dataLong[1] = count;
									sMetaAlbumCache.put(setId,
											longArrayToByteArray(dataLong), 0);
								}
							}
						}
	...
	private final static void populateMediaItemsForSets(final Context context,
			final ArrayList<MediaSet> sets,
		...
		try {
			...
			cursors[1] = cursorVideos;
			final SortCursor sortCursor = new SortCursor(cursors,
					Images.ImageColumns.DATE_TAKEN, SortCursor.TYPE_NUMERIC,
					true);
			if (Thread.interrupted()) {
				return;
			}
			try {
				if (sortCursor != null && sortCursor.moveToFirst()) {
					...
					do {
						...
						final MediaItem item = new MediaItem();
						final boolean isVideo = (sortCursor
								.getCurrentCursorIndex() == 1);
						if (isVideo) {
							populateVideoItemFromCursor(context, item, cr,
									sortCursor,
									CacheService.BASE_CONTENT_STRING_VIDEOS);
						} else {
							populateMediaItemFromCursor(context, item, cr,
									sortCursor,
									CacheService.BASE_CONTENT_STRING_IMAGES);
						}
						final long setId = sortCursor
								.getLong(MEDIA_BUCKET_ID_INDEX);
				...
				if (sortCursor != null)
					sortCursor.close();
				LocalDetectService.startLocalDetect(context);
			}
		} catch (Exception e) {
			// If the database operation failed for any reason
			e.printStackTrace();
	...
	private static final void writeSetsToCache(final ArrayList<MediaSet> sets) {
		...
		final DataOutputStream dos = new DataOutputStream(
				new BufferedOutputStream(bos, 256));
		try {
			...
			for (int i = 0; i < numSets; ++i) {
				...
				CacheUtils.writeUTF(dos, set.mName);
			...
			if (sAlbumCache.get(ALBUM_CACHE_NONEIMAGE_INDEX, 0) != null) {
				sAlbumCache.delete(ALBUM_CACHE_NONEIMAGE_INDEX);
			}
			dos.close();
			if (numSets == 0) {
				sAlbumCache.deleteAll();
				sAlbumCache.put(ALBUM_CACHE_NONEIMAGE_INDEX, sDummyData, 0);
				putLocaleForAlbumCache(Locale.getDefault());
			}
			sAlbumCache.flush();
		} catch (IOException e) {
		...
		try {
			...
			for (int i = 0; i < numItems; ++i) {
				...
				if (set.mId == LocalDataSource.CAMERA_BUCKET_ID
						|| set.mId == LocalDataSource.DOWNLOAD_BUCKET_ID) {
					// Reverse the display order for the camera bucket - want
					// the latest first.
					item = items.get(numItems - i - 1);
				}
				dos.writeLong(item.mId);
				CacheUtils.writeUTF(dos, item.mCaption);
				CacheUtils.writeUTF(dos, item.mMimeType);
				...
				CacheUtils.writeUTF(dos, item.mFilePath);
			}
			dos.flush();
	...
	private static final MediaSet findSet(final long id,
			final LongSparseArray<MediaSet> acceleratedTable) {
