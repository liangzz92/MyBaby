...
public final class LocalDataSource implements DataSource {
	...
	public static final DiskCache sThumbnailCache = new DiskCache(
			"local-image-thumbs");
	public static final DiskCache sThumbnailCacheVideo = new DiskCache(
			"local-video-thumbs");

	public static final String CAMERA_STRING = "Camera";
	public static final String DOWNLOAD_STRING = "download";
	public static final String CAMERA_BUCKET_NAME = CommonsUtil
			.getRootFilePath() + "/DCIM/" + CAMERA_STRING;
	public static final String DOWNLOAD_BUCKET_NAME = CommonsUtil
			.getRootFilePath() + "/" + DOWNLOAD_STRING;
	...
	public void loadMediaSets(final MediaFeed feed) {
		...
		stopListeners();
		CacheService
				.loadMediaSets(feed, this, !mDisableImages, !mDisableVideos);
		Handler handler = ((HomePageActivity) mContext).getHandler();
		ContentObserver observer = new ContentObserver(handler) {
			@Override
			public void onChange(boolean selfChange) {
				final boolean isPaused = ((HomePageActivity) mContext)
						.isPaused();
				if (isPaused) {
	...
	protected void refreshUI(MediaFeed feed, long setIdToUse) {
		...
		if (feed.getMediaSet(setIdToUse) == null) {
			...
			if (CommonsUtil.DEBUG) {
				Log.i(TAG, "Replacing mediaset " + mediaSet.mName + " id "
						+ setIdToUse + " current Id " + mediaSet.mId);
			}
			if (setIdToUse == CAMERA_BUCKET_ID) {
				mediaSet.mName = CAMERA_STRING;
			} else if (setIdToUse == DOWNLOAD_BUCKET_ID) {
	...
	@Override
	public void loadItemsForSet(final MediaFeed feed, final MediaSet parentSet,
	...
	private void loadMediaItemsIntoMediaFeed(final MediaFeed mediaFeed,
			final MediaSet set, int rangeStart, int rangeEnd) {
		if (rangeEnd - rangeStart < 0) {
			return;
		}
		CacheService.loadMediaItemsIntoMediaFeed(mediaFeed, set, rangeStart,
				rangeEnd, !mDisableImages, !mDisableVideos);
	...
	@Override
	public boolean performOperation(final int operation,
		...
		case MediaFeed.OPERATION_DELETE:
			for (int i = 0; i < numBuckets; ++i) {
				...
				if (set != null && items == null) {
					...
					final String whereImages = Images.ImageColumns.BUCKET_ID
							+ "=" + Long.toString(set.mId);
					final String whereVideos = Video.VideoColumns.BUCKET_ID
							+ "=" + Long.toString(set.mId);
		...
		try {
			...
			if (uriScheme.equals("file")) {
				...
				exif.setAttribute(ExifInterface.TAG_ORIENTATION, Integer
						.toString(Shared.degreesToExifOrientation(rotation)));
				exif.saveAttributes();
			}
	...
	public static MediaItem createMediaItemFromUri(Context context, Uri target) {
		...
		try {
			Cursor cursor = cr.query(Images.Media.EXTERNAL_CONTENT_URI,
					CacheService.PROJECTION_IMAGES, whereClause, null, null);
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					item = new MediaItem();
					CacheService.populateMediaItemFromCursor(context, item, cr,
							cursor,
	...
	public static MediaItem createMediaItemFromFileUri(Context context,
			String fileUri) {
		MediaItem item = null;
		String filepath = new File(URI.create(fileUri)).toString();
		ContentResolver cr = context.getContentResolver();
		long bucketId = parseBucketIdFromFileUri(fileUri);
		String whereClause = Images.ImageColumns.BUCKET_ID + "=" + bucketId
				+ " AND " + Images.ImageColumns.DATA + "='" + filepath + "'";
		try {
			Cursor cursor = cr.query(Images.Media.EXTERNAL_CONTENT_URI,
					CacheService.PROJECTION_IMAGES, whereClause, null, null);
			if (cursor != null) {
