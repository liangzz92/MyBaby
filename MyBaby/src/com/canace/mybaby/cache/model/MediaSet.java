...
import com.canace.mybaby.cache.utils.ArrayUtils;
...
public class MediaSet {
	...
	public void generateTitle(final boolean truncateTitle) {
		...
		String size = (mNumExpectedItemsCountAccurate) ? "  ("
				+ mNumExpectedItems + ")" : "";
		mTitleString = mName + size;
		if (truncateTitle) {
			int length = mName.length();
			mTruncTitleString = (length > 16) ? mName.substring(0, 12) + "..."
					+ mName.substring(length - 4, length) + size : mName + size;
