...
import com.canace.mybaby.cache.utils.ReverseGeocoder;
...
public final class MediaItem {
	...
	public String getDisplayMimeType() {
		if (mDisplayMimeType == null && mMimeType != null) {
			...
			if (slashPos != -1 && slashPos + 1 < mMimeType.length()) {
				mDisplayMimeType = mMimeType.substring(slashPos + 1)
						.toUpperCase();
			} else {
	...
	public String getReverseGeocodedLocation(ReverseGeocoder reverseGeocoder) {
		...
		mReverseGeocodedLocation = reverseGeocoder.getReverseGeocodedLocation(
				mLatitude, mLongitude, 2);
		return mReverseGeocodedLocation;
	}

	public long getTimeScore() {
		if (isDateAddedValid()) {
			return mDateTakenInMs;
		} else if (isDateModifiedValid()) {
			return mDateModifiedInSec;
		} else if (isDateAddedValid()) {
			return mDateAddedInSec;
		}
		return 0;
	}
}
