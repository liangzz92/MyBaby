package com.canace.mybaby.cache.model;
...
import com.canace.mybaby.cache.model.MediaClustering.Cluster;
import com.canace.mybaby.cache.utils.IndexRange;
import com.canace.mybaby.cache.utils.MediaFilter;
...
public final class MediaFeed implements Runnable {
	...
	private final ArrayList<MediaSet> mMediaSets = new ArrayList<MediaSet>();
	...
	private final HashMap<MediaSet, MediaClustering> mClusterSets = new HashMap<MediaSet, MediaClustering>(
			32);
	...
	public void setVisibleRange(int begin, int end) {
		if (begin != mVisibleRange.begin || end != mVisibleRange.end) {
			...
			mBufferedRange.begin = (begin / numItemsBy2) * numItemsBy2
					- numItemsBy4;
	...
	public MediaSet addMediaSet(final long setId, DataSource source) {
		MediaSet mediaSet = new MediaSet(source);
		mediaSet.mId = setId;
	...
	public MediaClustering getClustering() {
		if (mExpandedMediaSetIndex != Shared.INVALID
				&& mExpandedMediaSetIndex < mMediaSets.size()) {
	...
	public void addItemToMediaSet(MediaItem item, MediaSet mediaSet) {
		...
		synchronized (mClusterSets) {
			if (item.mClusteringState == MediaItem.NOT_CLUSTERED) {
				...
				clustering.setTimeRange(mediaSet.mMaxTimestamp
						- mediaSet.mMinTimestamp,
						mediaSet.getNumExpectedItems());
	...
	public void performOperation(final int operation,
			final ArrayList<MediaBucket> mediaBuckets, final Object data) {
		int numBuckets = mediaBuckets.size();
		final ArrayList<MediaBucket> copyMediaBuckets = new ArrayList<MediaBucket>(
				numBuckets);
		...
		Thread operationThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (operation == OPERATION_DELETE) {
					...
					mMediaFeedNeedsToRun = true;
					if (mDataSource != null) {
						mDataSource.performOperation(OPERATION_DELETE,
								mediaBuckets, null);
					}
				} else {
	...
	public int getNumSlots() {
		...
		if (mInClusteringMode == false) {
			if (currentMediaSetIndex == Shared.INVALID
					|| currentMediaSetIndex >= mediaSetsSize) {
				return mediaSetsSize;
			} else {
				MediaSet setToUse = (mMediaFilteredSet == null) ? mediaSets
						.get(currentMediaSetIndex) : mMediaFilteredSet;
				return setToUse.getNumItems();
			}
		} else if (currentMediaSetIndex != Shared.INVALID
				&& currentMediaSetIndex < mediaSetsSize) {
	...
	public MediaSet getSetForSlot(int slotIndex) {
		...
		if (mInClusteringMode == false) {
			...
			MediaSet setToUse = (mMediaFilteredSet == null) ? mMediaSets
					.get(currentMediaSetIndex) : mMediaFilteredSet;
			...
			MediaClustering clustering = mClusterSets.get(set);
			if (clustering != null) {
				ArrayList<MediaClustering.Cluster> clusters = clustering
						.getClustersForDisplay();
	...
	public void start() {
		...
		mAlbumSourceThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				while (ImageManager.isMediaScannerScanning(mContext
						.getContentResolver())) {
				...
				if (mWaitingForMediaScanner) {
					showToast(
							mContext.getResources().getString(
									R.string.loading_new), Toast.LENGTH_LONG);
					mWaitingForMediaScanner = false;
					if (dataSource != null) {
						dataSource.loadMediaSets(feed);
					}
	...
	private void showToast(final String string, final int duration,
		...
		if (mContext != null && !((MyBabyActivity) mContext).isPaused()) {
			((HomePageActivity) mContext).getHandler().post(new Runnable() {
				@Override
				public void run() {
					if (mContext != null) {
						Toast toast = Toast
								.makeText(mContext, string, duration);
						if (centered) {
		...
		if (dataSource != null) {
			while (!Thread.interrupted()) {
				...
				if (!mMediaFeedNeedsToRun)
					continue;
				if (((MyBabyActivity) mContext).isPaused())
					continue;
				...
				synchronized (mediaSets) {
					...
					if (expandedSetIndex == Shared.INVALID) {
						...
						for (int i = 0; i < numSets; ++i) {
							MediaSet set = mediaSets.get(i);
							if (i >= bufferedRange.begin
									&& i <= bufferedRange.end) {
								if (scanMediaSets) {
									int numItemsLoaded = set.mNumItemsLoaded;
									if (numItemsLoaded < set
											.getNumExpectedItems()
											&& numItemsLoaded < 8) {
										dataSource.loadItemsForSet(this, set,
												numItemsLoaded, 8);
										...
										if (mListener != null) {
											mListener
													.onFeedChanged(this, false);
										}
										sleepMs = 100;
										scanMediaSets = false;
									}
								}
							} else if (i < bufferedRange.begin
									|| i > bufferedRange.end) {
								// Purge this set to its initial status.
								MediaClustering clustering = mClusterSets
										.get(set);
								if (clustering != null) {
									clustering.clear();
					...
					if (expandedSetIndex != Shared.INVALID) {
						...
						if (mInClusteringMode && mClusterSets != null) {
							requestedItems = 0;
							MediaClustering clustering = mClusterSets
									.get(mediaSets.get(expandedSetIndex));
							if (clustering != null) {
								ArrayList<Cluster> clusters = clustering
										.getClustersForDisplay();
								int numClusters = clusters.size();
								for (int i = 0; i < numClusters; i++) {
									requestedItems += clusters.get(i)
											.getNumExpectedItems();
								}
							}
						}
						...
						if (numItemsLoaded < set.getNumExpectedItems()) {
							...
							dataSource.loadItemsForSet(this, set,
									numItemsLoaded,
									(requestedItems / NUM_ITEMS_LOOKAHEAD)
											* NUM_ITEMS_LOOKAHEAD
											+ NUM_ITEMS_LOOKAHEAD);
							if (set.getNumExpectedItems() == 0) {
								mediaSets.remove(set);
							...
							if (numItemsLoaded != set.mNumItemsLoaded
									&& mListener != null) {
								mListener.onFeedChanged(this, false);
							}
						}
	...
	public void expandMediaSet(int mediaSetIndex) {
		...
		if (mExpandedMediaSetIndex > 0 && mediaSetIndex == Shared.INVALID) {
			...
			if (mediaSetIndex < mMediaSets.size()
					&& mExpandedMediaSetIndex >= 0
					&& mExpandedMediaSetIndex < mMediaSets.size()) {
