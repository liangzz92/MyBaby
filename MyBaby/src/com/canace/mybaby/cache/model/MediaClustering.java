...
import com.canace.mybaby.cache.utils.LocationMediaFilter;
...
public final class MediaClustering {
	...
	public void setTimeRange(long timeRange, int numItems) {
		...
		mClusterSplitTime = Shared.clamp(mClusterSplitTime,
				MIN_CLUSTER_SPLIT_TIME_IN_MS, MAX_CLUSTER_SPLIT_TIME_IN_MS);
		mLargeClusterSplitTime = mClusterSplitTime
				/ PARTITION_CLUSTER_SPLIT_TIME_FACTOR;
		mMinClusterSize = Shared.clamp(mMinClusterSize, MIN_MIN_CLUSTER_SIZE,
				MAX_MIN_CLUSTER_SIZE);
		mMaxClusterSize = Shared.clamp(mMaxClusterSize, MIN_MAX_CLUSTER_SIZE,
	...
	private int getPartitionIndexForCurrentCluster() {
		...
		if (numCurrClusterItems > minClusterSize + 1) {
			for (int i = minClusterSize; i < numCurrClusterItems
				...
				if (prevItem.isDateTakenValid()
						&& currItem.isDateModifiedValid()
						&& nextItem.isDateModifiedValid()) {
					long diff1 = Math.abs(nextItem.mDateTakenInMs
							- currItem.mDateTakenInMs);
					long diff2 = Math.abs(currItem.mDateTakenInMs
							- prevItem.mDateTakenInMs);
	...
	public static final class Cluster extends MediaSet {
		...
		public void generateCaption(Context context) {
			if (mClusterChanged) {
				...
				if (minTimestamp != -1L) {
					if (mIsPicassaAlbum) {
						minTimestamp -= HomePageActivity.CURRENT_TIME_ZONE
								.getOffset(minTimestamp);
						maxTimestamp -= HomePageActivity.CURRENT_TIME_ZONE
								.getOffset(maxTimestamp);
					}
					String minDay = DateFormat.format(MMDDYY_FORMAT,
					...
					if (minDay.substring(4).equals(maxDay.substring(4))) {
						...
						mName = DateUtils.formatDateRange(context,
						...
						if (minDay.equals(maxDay)) {
							...
							String dateRangeWithOptionalYear = DateUtils
									.formatDateTime(context, minTimestamp,
											flags);
							String dateRangeWithYear = DateUtils
									.formatDateTime(context, minTimestamp,
											flags | DateUtils.FORMAT_SHOW_YEAR);
							if (!dateRangeWithOptionalYear
									.equals(dateRangeWithYear)) {
						...
						int flags = DateUtils.FORMAT_NO_MONTH_DAY
								...
								| DateUtils.FORMAT_SHOW_DATE;
						mName = DateUtils.formatDateRange(context,
								minTimestamp, maxTimestamp, flags);
					}
				} else {
	...
	private static boolean isGeographicallySeparated(MediaItem a, MediaItem b) {
		...
		if (a != null && b != null && a.isLatLongValid() && b.isLatLongValid()) {
			int distance = (int) (LocationMediaFilter
					.toMile(LocationMediaFilter.distanceBetween(a.mLatitude,
							a.mLongitude, b.mLatitude, b.mLongitude)) + 0.5);
