...
public class SortCursor extends AbstractCursor {
	...
	public SortCursor(Cursor[] cursors, String sortcolumn, int type,
			boolean ascending) {
		...
		if (type == TYPE_STRING) {
			...
			for (int j = 0; j < length; j++) {
				...
				boolean comparison = (ascending) ? (current < smallest)
						: (current > smallest);
		...
		if (mType == TYPE_STRING) {
			...
			for (int i = oldPosition; i <= newPosition; i++) {
				...
				for (int j = 0; j < length; j++) {
					...
					long current = mCursors[j].getLong(mSortColumns[j]);
					boolean comparison = (mAscending) ? current < smallest
							: current > smallest;
