...
public class LocationMediaFilter extends MediaFilter {
	...
	LocationMediaFilter(double centerLatitude, double centerLongitude,
	...
	LocationMediaFilter(double latitude1, double longitude1, double latitude2,
			double longitude2) {
	...
	private static final double centerOfAngles(double ang1, double ang2,
			int wrapAroundThreshold) {
	...
	public boolean pass(MediaItem item) {
		double radius = distanceBetween(mCenterLat, mCenterLon, item.mLatitude,
				item.mLongitude);
		if (radius <= mRadius) {
			return true;
		}
